...
FLAGS = flags.FLAGS
flags.DEFINE_string("model_dir", "XPL/XPL/", "Where to save the checkpoints.")
...
def main(argv):
    ...
    with mirrored_strategy.scope():
        ...
        def decode_object_loss(new_image, mask_sum, recons, depth):
            ...
            loss = tf.math.squared_difference(recons_image, new_image)
            return tf.reduce_sum(loss, axis=[2, 3, 4]), recons * masks
        ...
        def test_step(batch, model_reason, model_fast, model_new, step="test"):
            ...
            objects = objects_pre * mask_mean + objects_init * (1 - mask_mean)
    ...
    with mirrored_strategy.scope():
        ...
        def distributed_test_step(data,
            ...
            per_replica_losses = mirrored_strategy.run(
                test_step,
                args=(data, model_reason, model_fast, model_new, step))
        ...
        start = time.time()
        name_list = ["collision", "blocking", "permanence", "continuity"]
        test_data_list = [
            collision_ds, blocking_ds, permanence_ds, continuity_ds
        ]
        model_name = "XPL"
        figure_dir = "./output/"
        for i in range(4):
            test_ds = test_data_list[i]
            type_name = name_list[i]
            filename = figure_dir + model_name + "/" + type_name
            feature_data = None
            out_data = None
            for batch, data in enumerate(test_ds):
                objects, objects_2, image, mask = distributed_test_step(
                    data, model_reason, model_fast, model_new, step="test")
                feature = {
                    "image": image,
                    "mask": mask,
                    "objects": objects,
                ...
                for batch, data in enumerate(feature_data):
                    loss1, loss2, eval_all = distributed_loss_step(
                        data, model_dec)
                    out = {
                        "loss1": loss1,
                        "loss2": loss2,
                ...
                del feature_data
            tf.data.experimental.save(out_data, filename, compression='GZIP')
            ...
            ds = cal_relative(test,
                              ...
                              model_name=model_name,
                              figure_dir=figure_dir)
            log_fn("test: {}, Time: {}".format(
                type_name, datetime.timedelta(seconds=time.time() - start)))
